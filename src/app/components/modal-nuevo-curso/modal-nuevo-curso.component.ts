...
import { curso, EntrenadorService } from 'src/app/service/entrenador.service';
...
@Component({
  selector: 'app-modal-nuevo-curso',
  templateUrl: './modal-nuevo-curso.component.html',
  styleUrls: ['./modal-nuevo-curso.component.scss']
})
export class ModalNuevoCursoComponent implements OnInit {
  resultado!:any;
...
/*
  ...
  */
  constructor(private formBuilder:FormBuilder, private servicio:EntrenadorService,private toastr:ToastrService,
  ...
  enviarReactivo() {
    ...
    console.log(curso);
    this.servicio.postCurso(curso).subscribe( resp => {
      this.toastr.success("Se agrego correctamente el Curso","Nuevo Curso!!!");
      console.log(resp);
        setTimeout(function(){
      ...
      this.toastr.error("Debe completar los campos","Error al cargar curso");
    }
...
/*      let  imageSinDescripcion = this.image.replace('data:image/jpeg;base64,/9j/','');
      this.vistaImagen = await imageSinDescripcion;
      this.resultado = "https://www.greatplacetowork.com.ar/templates/gptw/images/no-image-available.jpg"+this.vistaImagen;
*/
  ...
  readThis(event: any): void {
    ...
    myReader.onloadend = e => {
      ...
      this.servicio.imgbbPost(imageSinDescripcion).subscribe(resp=>{
        console.log(resp);
      })
    }
  }
